...
\begin{block}{Unit Testing (PyUnit)}
  \textbf{55 tests} across ten classes, each traced to a requirement
  from Deliverable~2. Six of the ten:
  \vspace{0.3cm}
  ...
  \raggedright
  \textbf{Validated by mutation.} Twenty defects were injected into the
  implementation; the suite detected seventeen. Of the three survivors,
  all were behaviour-preserving.
  \vspace{0.2cm}

  Two earlier survivors revealed genuine weaknesses --- an unreachable
  validation branch masked by a later guard, and a magnitude boundary
  asserted relative to the constant it was meant to pin. Both are now
  covered.
\end{block}

\begin{block}{Quality Tooling}
  \centering
...
\begin{block}{Debugger: Terms Shrinking to Tolerance}
  \centering
...
\begin{block}{Which UI Principles Apply?}
  \centering
...
\begin{block}{Accessibility}
  ...
  \begin{itemize}
    ...
    \item Contrast measured at 6.9:1 and 7.1:1 against the 4.5:1
          requirement
...
\begin{block}{Generative AI Use}
  ...
  The test review found three real weaknesses --- a vacuous assertion,
  an unreachable branch, and a boundary asserted relative to the
  constant it was meant to pin. All are now covered. Prompts and
  responses are in \texttt{screenshots/}.
\end{block}
...
\begin{block}{Repository}
  ...
  {\small Version \texttt{v1.0.0} --- Semantic Versioning}
\end{block}
